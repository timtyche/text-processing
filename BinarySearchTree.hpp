...
private:
  ...
  static Node *copy_nodes_impl(Node *node)
  {
    ...
    {
      n = new Node(node->datum, copy_nodes_impl(node->left), 
  ...
  static Node *insert_impl(Node *node, const T &item, Compare less)
{
    ...
    // Recursive case: if the item is less than the current node's datum,
    // insert it into the left subtree; otherwise, insert it into the right
    // subtree.
